...
func TestStopCausesJobsToNotRun(t *testing.T) {
	...
	cron.Start()
	cron.Stop()
	cron.AddFunc("test job", "* * * * * ?", func() { wg.Done() })
...
func TestAddWhileRunningWithDelay(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("test job", "* * * * * *", func() { atomic.AddInt64(&calls, 1) })
...
func TestRemoveBeforeRunning(t *testing.T) {
	...
	cron := newWithSeconds()
	id, _ := cron.AddFunc("test job", "* * * * * ?", func() { wg.Done() })
	cron.Remove(id)
	cron.Start()
	defer cron.Stop()
...
func TestSnapshotEntries(t *testing.T) {
	...
	cron := New()
	cron.AddFunc("test job", "@every 2s", func() { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("test job", "0 0 0 1 1 ?", func() {})
	cron.AddFunc("test job 1", "* * * * * ?", func() { wg.Done() })
	id1, _ := cron.AddFunc("test job 2", "* * * * * ?", func() { t.Fatal() })
	id2, _ := cron.AddFunc("test job 3", "* * * * * ?", func() { t.Fatal() })
	cron.AddFunc("test job 4", "0 0 0 31 12 ?", func() {})
	cron.AddFunc("test job 5", "* * * * * ?", func() { wg.Done() })
...
func TestRunningJobTwice(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("test job1", "0 0 0 1 1 ?", func() {})
	cron.AddFunc("test job2", "0 0 0 31 12 ?", func() {})
	cron.AddFunc("test job3", "* * * * * ?", func() { wg.Done() })
...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	cron.AddFunc("test job2", "0 0 0 31 12 ?", func() {})
	cron.AddFunc("test job3", "* * * * * ?", func() { wg.Done() })
	cron.Schedule("test job4", Every(time.Minute), FuncJob(func() {}))
	cron.Schedule("test job5", Every(time.Second), FuncJob(func() { wg.Done() }))
	cron.Schedule("test job6", Every(time.Hour), FuncJob(func() {}))
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddFunc("test job", spec, func() { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestInvalidJobSpec(t *testing.T) {
	cron := New()
	_, err := cron.AddJob("test job", "this will not parse", nil)
	if err == nil {
...
func TestJob(t *testing.T) {
	...
	cron := newWithSeconds()
	cron.AddJob("test job1", "0 0 0 30 Feb ?", testJob{wg, "job0"})
	cron.AddJob("test job2", "0 0 0 1 1 ?", testJob{wg, "job1"})
	job2, _ := cron.AddJob("test job3", "* * * * * ?", testJob{wg, "job2"})
	cron.AddJob("test job3", "1 0 0 1 1 ?", testJob{wg, "job3"})
	cron.Schedule("test job4", Every(5*time.Second+5*time.Nanosecond), testJob{wg, "job4"})
	job5 := cron.Schedule("test job5", Every(5*time.Minute), testJob{wg, "job5"})
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	cron := newWithSeconds()
	hourJob := cron.Schedule("test job1", Every(time.Hour), FuncJob(func() {}))
	cron.Schedule("test job2", Every(time.Second), FuncJob(func() {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	cron := newWithSeconds()
	var calls int64
	cron.AddFunc("test job1", "* * * * * *", func() { atomic.AddInt64(&calls, 1) })
	cron.Schedule("test job2", new(ZeroSchedule), FuncJob(func() { t.Error("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs added, still returns immediately", func(t *testing.T) {
		cron := newWithSeconds()
		cron.AddFunc("test job1", "* * * * * *", func() {})
		cron.Start()
		cron.AddFunc("test job2", "* * * * * *", func() {})
		cron.AddFunc("test job3", "* * * * * *", func() {})
		cron.AddFunc("test job4", "* * * * * *", func() {})
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.AddFunc("test job1", "* * * * * *", func() {})
		cron.Start()
		cron.AddFunc("test job2", "* * * * * *", func() { time.Sleep(2 * time.Second) })
		cron.AddFunc("test job3", "* * * * * *", func() {})
		time.Sleep(time.Second)
